;; init-workspace.el --- Initialize prog configurations.	-*- lexical-binding: t -*-
;;
...
(use-package persp-mode
  ...
  (setq persp-autokill-buffer-on-remove 'kill-weak
        ...
        persp-save-dir (concat sea-etc-dir "workspaces/")
  ...
  ;; Bootstrap
  (add-hook 'after-init-hook #'+workspaces|init)
...
Ensures the scratch (or dashboard) buffers are CDed into the project's root."
  ...
  (defun +workspaces|on-persp-mode ()
    ;; Remap `buffer-list' to current workspace's buffers in `sea-buffer-list'
    (if persp-mode
        (advice-add #'sea-buffer-list :override #'+workspace-buffer-list)
      (advice-remove #'sea-buffer-list #'+workspace-buffer-list)))
